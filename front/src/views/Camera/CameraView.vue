  ...
  toggleCamera() { // 열기, 닫기 버튼클릭 시 호출 메소드
	...
	async sendingToServer(){
		...
		try {
			...
			await axios.post("http://192.168.0.115:9000/food_ai/detectFoodWeb", formData)
			.then((res) => {
	...
	async uploadToServer_web(){
		if(this.uploadFile !== null){
			...
			try {
				this.isLoadingImg = true;
				await axios.post("http://192.168.0.115:9000/food_ai/detectFoodWebUpload", formData, {headers:{'Content-Type':'multipart/form-data'}})
				.then((res) => {
					console.log("전송이 성공적으로 완료")
					console.log(res.data);
	...
	uploadToDB_web(){
		...
		try {
			this.isLoadingImg = true;
			axios.post("http://192.168.0.115:9000/food_ai/detectFoodWebUpload_save", formData)
			.then((res) => {
				this.isLoadingImg = false;
				if (res.data.res_code === "1"){
					Swal.fire({
